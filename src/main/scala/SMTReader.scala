...
class SMTReader(fileName: String) {
  ...
  private val settings =
    Param.BOOLEAN_FUNCTIONS_AS_PREDICATES.set(ParserSettings.DEFAULT, true)
  ...
  val (wordConstants, otherConstants) =
    allConstants partition { c =>
      parser.env.lookupSym(c.name) match {
        case Environment.Constant(_, _, SMTLIBStringParser.SMTSeq(_)) => true
        case _                                                        => false
      }
    }
...
//debug----------------
  ...
  def dnf_base(
      dnf1: List[ap.parser.IFormula],
      dnf2: List[ap.parser.IFormula]
  ) = {
  ...
  def cnf_base(
      cnf1: List[ap.parser.IFormula],
      cnf2: List[ap.parser.IFormula]
  ) = {
  ...
  def containsPredicate(aF: IFormula): Boolean = {
    aF match {
      ...
      case IBinFormula(j, f1, f2) =>
        containsPredicate(f1) || containsPredicate(f2)
