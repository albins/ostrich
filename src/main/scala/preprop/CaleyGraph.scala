...
class Box[A <: AtomicStateAutomaton] {
  ...
  def ++(that: Box[A]): Box[A] =
    getEdges.foldLeft(new Box[A]) {
      case (box, (q1, q2)) =>
        that(q2).foldLeft(box)((box, q3) => box.addEdge(q1, q3))
    }
...
object CaleyGraph {
  ...
  def apply[A <: AtomicStateAutomaton](
      aut: A,
      wordBounds: Seq[AtomicStateAutomaton] = Seq()
  ): CaleyGraph[A] = Exploration.measure("build caley graph") {
      ...
      for ((box, as) <- characterBoxes) {
        ...
        for ((bsNext, blbl) <- boundAut.outgoingTransitions(bs);
             if as.exists(
               a =>
                 boundAut.LabelOps
                   .labelsOverlap(blbl, a.asInstanceOf[boundAut.TLabel])
             )) {
  ...
  private def getCharacterBoxes[A <: AtomicStateAutomaton](
      aut: A
