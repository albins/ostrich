        ...
        val argLengths =
          (for (argAuts <- argumentConstraints.iterator) yield {
            (for (aut <- argAuts.iterator;
                  ...
                  l <- aut
                    .asInstanceOf[AtomicStateAutomaton]
                    .uniqueAcceptedWordLength
                    .iterator)
          ...
          case Some((lenAut, len)) =>
            ...
            (
              for (s <- resultConstraint.states.iterator;
                ...
                val splitA =
                  InitFinalAutomaton.setFinal(resultConstraint, Set(s))
                ...
                for (i <- 0 to splitA.registers.length - 1) {
                  ...
                  val internRes =
                    AtomicStateAutomatonAdapter.intern(resultConstraint)
                  ...
                  for (s <- resultConstraint.states.iterator;
                    ...
                    for (i <- 0 to splitA.registers.length - 1) {
                      val internRes =
                        AtomicStateAutomatonAdapter.intern(resultConstraint)
                      val r =
                        internRes.asInstanceOf[BricsAutomaton].registers(i)
                      val r1 = splitA.registers(i)
                      val r2 = splitB.registers(i)
                      a.addFormula(r === r1 + r2)
                    }
